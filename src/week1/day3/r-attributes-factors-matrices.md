...
**Exercise.** Write a function that creates a matrix of specified dimensions where the value in row $i$, column $j$ is equal to $\mathrm{min}(i,j)$.
...
**Exercise.** Turn a list into a matrix, forming a list-matrix capable of holding different data types. (*Hint:* Use `dim()`.) Speculate on some use cases of list-matrices.
...
**Exercise.** What's the physical interpretation of the matrices generated by `mystery()`? Consider what happens when you multiply particular column vectors (matrices with multiple rows but just a single column), notably those with 0 as one element and 1 as the other element. You can graph how the values of each element in these column vectors change after multiplication by the output of `mystery(x)` with respect to `x`.
...
[^rot]: Calling `mystery(x)` returns a 2-dimensional rotation matrix corresponding to rotation through the angle `x` (given in radians). We have `mystery(0) != mystery(2*pi)` because of floating-point imprecision.