...
**Exercise.** Consider the matrices given by `mystery = function(x) matrix(c(cos(x), -sin(x), sin(x), cos(x)), nrow=2)`. Is the `mystery()` function *periodic* in some sense? Check if this holds in practice; if there's a discrepancy, explain it. What's the physical interpretation of the matrices generated by `mystery()`? (*Hint:* Consider what happens when you multiply particular vectors with these matrices.)[^rot]
