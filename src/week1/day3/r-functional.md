...
* `sapply()` tries to *guess the best output type*. Operating on the columns of a data frame, `sapply()` will bind the results together into a new data frame. When appropriate, `sapply()` will `unlist()` a list of results into a vector.su
